...
#group for ablation_model7
group2_all=[]
for i in group2:
    group2_all=group2_all+i
group2_all=[group2_all]
    
group3_all=[1]
group3_all=[group3_all]

#group for ablation_model8 and 9
group1_all=[]
for i in group1:
    group1_all=group1_all+i
group1_all=[group1_all]

group2_all_=[1]
group2_all_=[group2_all_]
...
raw=pd.read_csv('./data/raw57_2.csv').drop(['time'],axis=1)
...
for i in range(len(data)):
    data_sw.append([data[i].iloc[j:j+sw_size,:].T for j in range(0,data[0].shape[0]-sw_size,sw_step)])
...
# dense
def ablation_attention_layer(X, n_h, Ty):
    output=keras.layers.Reshape((keras.backend.int_shape(X)[1]*keras.backend.int_shape(X)[2],))(X)
    output=[keras.layers.Dense(n_h)(output)]
    
    attention=keras.layers.Dense(1)(X)
    attention=tf.ones_like(attention)
...
for i in range(len(group1)):
    ...
    # attention1_output1,attention1_output2=attention_layer(orth1[i], 64, 1)
    # ablate attention layer 1
    attention1_output1,attention1_output2=ablation_attention_layer(orth1[i], 64, 1)
...
for i in range(len(group2)):
    if len(group2[i])==1:
        ...
        # attention2_output1,attention2_output2=attention_layer(comb1[i], 64, 1)
        # ablate attention layer 2
        attention2_output1,attention2_output2=ablation_attention_layer(comb1[i], 64, 1)
        attention2_layer.append(attention2_output1)
        attention2.append(attention2_output2)
        temp_input=[]
        for j in group2[i]:
...
for i in range(8):
    # attention3_output1,attention3_output2=attention_layer(comb2[0], 64, 1)
    # ablate attention layer 3
    attention3_output1,attention3_output2=ablation_attention_layer(comb2[0], 64, 1)
    attention3_layer.append(attention3_output1)
    attention3.append(attention3_output2)
    temp_input=[]
    for j in range(len(group1)):
...
# attention4_layer,attention4=attention_layer(comb3[0], 64, 1)
# ablate attention layer 2
attention4_layer,attention4=ablation_attention_layer(comb3[0], 64, 1)
...
################################################################################################################
##ablation_model5
ablate attention layer 3 and 4
################################################################################################################
'''
...
for i in range(len(group1)):
    ...
    # orth1.append(keras.layers.Lambda(orth_layer)(lstm1[i])) 
    # ablate orth
    orth1.append(keras.layers.Lambda(ablation_orth_layer)(lstm1[i]))
...
comb1=[]
for i in range(len(group2_all)):
    if len(group2_all[i])==1:
        comb1.append([])
    else:
        temp_output=[]
        for j in group2_all[i]:
            temp_output.append(model1[j-1].output)
        comb1.append(keras.layers.Reshape((len(group2_all[i]),64))(keras.layers.Concatenate()(temp_output)))
...
for i in range(len(group2_all)):
    if len(group2_all[i])==1:
        attention2_layer.append([])
        attention2.append([])
        model2.append(model1[group2_all[i][0]-1])
    else:
...
comb2=[]
# temp_output=[]
# for j in group3_all[0]:
    # temp_output.append(model2[j-1].output)
# comb2.append(keras.layers.Reshape((len(group3_all[0]),64))(keras.layers.Concatenate()(temp_output)))
comb2.append(keras.layers.Reshape((len(group3_all[0]),64))(model2[0].output))
...
'''
################################################################################################################
ablation_model8 and ablation_model9
################################################################################################################
'''
...
for i in range(len(group1_all)):
    input1.append(keras.layers.Input(batch_shape=(None,len(group1_all[i]),sw_size)))      
    normal.append(keras.layers.BatchNormalization(axis=-1)(input1[i]))    
    lstm1.append(keras.layers.Bidirectional(keras.layers.LSTM(32, return_sequences=True), merge_mode='concat')(normal[i]))
...
comb1=[]
for i in range(len(group2_all_)):
    if len(group2_all_[i])==1:
        comb1.append([])
    else:
        temp_output=[]
        for j in group2_all_[i]:
            temp_output.append(model1[j-1].output)
        comb1.append(keras.layers.Reshape((len(group2_all_[i]),64))(keras.layers.Concatenate()(temp_output)))
...
for i in range(len(group2_all_)):
    if len(group2_all_[i])==1:
        attention2_layer.append([])
        attention2.append([])
        model2.append(model1[group2_all_[i][0]-1])
    else:
...
################################################################################################################
##ablation_model9
ablate attention 2 and 3 and 4
################################################################################################################
'''

x_input=keras.layers.Input(batch_shape=(None,191,sw_size))
x=keras.layers.BatchNormalization(axis=-1)(x_input)
x=keras.layers.Bidirectional(keras.layers.LSTM(32, return_sequences=True), merge_mode='concat')(x)

attention_output1,attention_output2=attention_layer(x, 64, 1)

dense1 = keras.layers.Dense(32, activation="relu")(attention_output1[0])
dense2 = keras.layers.Dense(8, activation="relu")(dense1)
dense3 = keras.layers.Dense(2, activation="sigmoid")(dense2)

model=keras.models.Model(inputs=x_input,outputs=dense3)
